#!/usr/bin/env python3
"""Apply checked, one-way MongoDB cutover edits to the large weapon files."""
...
def replace_exact(content: str, before: str, after: str, label: str) -> str:
    ...
    if before not in content:
        raise RuntimeError(f"Could not find migration block: {label}")
    return content.replace(before, after, 1)


def replace_regex(content: str, pattern: str, replacement: str, label: str, marker: str = "") -> str:
    if marker and marker in content:
        return content
    regex = re.compile(pattern, re.S)
    if not regex.search(content):
        raise RuntimeError(f"Could not find migration block: {label}")
    return regex.sub(lambda _match: replacement, content, count=1)


def migrate_weapon_interface() -> None:
    relative = "src/commands/commandList/battle/WeaponInterface.js"
    ...
    content = replace_regex(
        content,
        r"\t/\*\* Saves weapon to the db and return uwid \*\*/\n\tasync save\(id\) \{.*?\n\t\}\n\n\t/\*\* Update weapon in db \*\*/",
...
def migrate_weapon_util() -> None:
    relative = "src/commands/commandList/battle/util/weaponUtil.js"
    content = read(relative)
    original = content
    ...
    if "async function hydrateWeaponRows(weaponDocs)" not in content:
        marker = "}, 0);\n\nconst getRandomWeapon"
        if marker not in content:
            raise RuntimeError("Could not insert weapon MongoDB helpers")
        content = content.replace(marker, "}, 0);\n" + WEAPON_HELPERS + "\nconst getRandomWeapon", 1)

    content = replace_regex(
        content,
        r"exports\.getItems = async function \(p\) \{.*?\n\};\n\nlet parseWeapon",
        r'''exports.getItems = async function (p) {
...
let parseWeapon''',
    ...
    content = replace_regex(
        content,
        r"let getDisplayPage = async function \(p, user, page, sort, opt = \{\}\) \{.*?\n\t/\* Parse all weapons \*/\n\tlet user_weapons = parseWeaponQuery\(result\[0\]\);",
        r'''let getDisplayPage = async function (p, user, page, sort, opt = {}) {
	let { wid, widList } = opt;
    ...
    content = replace_regex(
        content,
        r"exports\.equip = async function \(p, uwid, pet\) \{.*?\n\};\n\nexports\.unequip",
        r'''exports.equip = async function (p, uwid, pet) {
...
/* Shorten a uwid''',
    ...
    content = replace_regex(
        content,
        r"exports\.getWeapon = async function \(uwid, id\) \{.*?\n\};",
        r'''exports.getWeapon = async function (uwid, id) {
	if (!uwid) return null;
	const collection = await mongo.collection('user_weapon');
...
def migrate_alter_battle() -> None:
    relative = "src/commands/commandList/patreon/alterBattle.js"
    content = read(relative)
    original = content
    content = replace_regex(
        content,
        r"\tconst sql = `SELECT alterbattle\.\* from alterbattle INNER JOIN user ON alterbattle\.uid = user\.uid WHERE user\.id = \$\{p\.msg\.author\.id\} AND alterbattle\.type = '\$\{type\}'`;\n\tconst result = \(await p\.query\(sql\)\)\[0\];",
...
def print_give_xp_references() -> None:
    print("giveXP references:")
    for file in (ROOT / "src").rglob("*.js"):
        for number, line in enumerate(file.read_text().splitlines(), 1):
            if "giveXP(" in line:
                print(f"  {file.relative_to(ROOT)}:{number}: {line.strip()}")


migrate_weapon_interface()
migrate_weapon_util()
migrate_alter_battle()
print_give_xp_references()
print(f"MongoDB weapon codemod changed {changed} file(s).")
