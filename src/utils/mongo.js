...
async function ensureIndexes(db) {
	...
	await Promise.all([
		...
		db.collection('user_announcement').createIndex({ uid: 1 }, { unique: true }),
		db.collection('vote').createIndex({ id: 1 }, { unique: true }),
		db.collection('marriage').createIndex({ uid1: 1 }),
		db.collection('marriage').createIndex({ uid2: 1 }),
